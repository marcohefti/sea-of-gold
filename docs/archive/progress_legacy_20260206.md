...
- 2026-02-06: Session-bootstrap docs audit + AGENTS headless policy update.

Decision:
- Added an explicit browser test policy to `AGENTS.md`: all automated browser validation runs must be headless.
- Rationale: avoids accidental visible browser popups and keeps validation behavior consistent across autonomous runs.

Validation run:
- `node scripts/run_autonomous_validation.mjs --scenarios smoke --skip-playability-check true --skip-repo-checks true --out-dir .codex-artifacts/idle-game/docs_headless_rule_smoke_20260206`
- Result: PASS (`check:autonomy-contract` OK, `smoke` PASS, `check:harness-artifacts` OK)
- Artifacts: `.codex-artifacts/idle-game/docs_headless_rule_smoke_20260206`
